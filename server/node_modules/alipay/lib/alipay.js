...
Alipay.prototype.create_direct_pay_by_user = function(data, res){
    ...
    var html_text = alipaySubmit.buildRequestForm(parametetrade_create_by_buyer_return_urlr,"get", "确认");
    res.send(html_text);
}
...
Alipay.prototype.refund_fastpay_by_platform_pwd = function(data, res){
    ...
    var html_text = alipaySubmit.buildRequestForm(parameter,"get", "跳转到支付宝");
    res.send(html_text);
}
...
Alipay.prototype.create_partner_trade_by_buyer = function(data, res){
    ...
    var html_text = alipaySubmit.buildRequestForm(parameter,"get", "正在跳转到支付宝");
    res.send(html_text);
}
...
Alipay.prototype.trade_create_by_buyer = function(data, res){
    ...
    var parameter = {
        ...
        notify_url  : this.alipay_config.host + "/alipay/notify_url",
        return_url :  this.alipay_config.host + "/alipay/return_url",
        seller_email    : this.alipay_config.seller_email, 
    ...
    var html_text = alipaySubmit.buildRequestForm(parameter,"get", "正在跳转到支付宝收银台...");
    res.send(html_text);
}
...
Alipay.prototype.trade_create_by_buyer_return = function(req, res) {
    ...
    alipayNotify.verifyReturn( _GET , function( verify_result ) {
        if( verify_result ) {//验证成功
            res.redirect( self.alipay_config.host );
            return;
            //商户订单号
            ...
            if(trade_status  == 'WAIT_BUYER_PAY'){                
                self.emit('trade_create_by_buyer_wait_buyer_pay', "return" , out_trade_no, trade_no , res );
            }
            else if(trade_status == 'WAIT_SELLER_SEND_GOODS'){       
                self.emit('trade_create_by_buyer_wait_seller_send_goods', "return" , out_trade_no, trade_no , res );
            }
            else if(trade_status == 'WAIT_BUYER_CONFIRM_GOODS'){                
                self.emit('trade_create_by_buyer_wait_buyer_confirm_goods', "return" , out_trade_no, trade_no , res );
            }
            else if(trade_status == 'TRADE_FINISHED'){                
                self.emit('trade_create_by_buyer_trade_finished', "return" , out_trade_no, trade_no , res );
            }
...
Alipay.prototype.trade_create_by_buyer_notify = function(req, res){
    ...
    alipayNotify.verifyNotify(_POST , function( verify_result ) {
          if(verify_result) {//验证成功
            ...
            if(trade_status  == 'WAIT_BUYER_PAY'){                
                self.emit('trade_create_by_buyer_wait_buyer_pay', "notify" , out_trade_no, trade_no , res );
            }
            else if(trade_status == 'WAIT_SELLER_SEND_GOODS'){                
                self.emit('trade_create_by_buyer_wait_seller_send_goods', "notify" , out_trade_no, trade_no , res );
            }
            else if(trade_status == 'WAIT_BUYER_CONFIRM_GOODS'){                
                self.emit('trade_create_by_buyer_wait_buyer_confirm_goods', "notify" , out_trade_no, trade_no , res );
            }
            else if(trade_status == 'TRADE_FINISHED'){                
                self.emit('trade_create_by_buyer_trade_finished', "notify" , out_trade_no, trade_no , res );
            }
